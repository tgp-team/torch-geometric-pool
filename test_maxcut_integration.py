...
def test_maxcut_select():
    ...
    try:
        ...
        if hasattr(select_output, "scores"):
            ...
            print(
                f"✓ Scores range: [{select_output.scores.min():.3f}, {select_output.scores.max():.3f}]"
            )
...
def test_score_network_kwargs():
    ...
    try:
        ...
        print(f"✓ Loss: {out.get_loss_value('maxcut_loss'):.4f}")
        print(
            f"✓ Loss coefficient applied: {abs(out.get_loss_value('maxcut_loss')) > 0}"
        )
...
def test_selectoutput_data():
    ...
    try:
        ...
        print(
            f"✓ Extra args contain required keys: {expected_extra.issubset(actual_extra)}"
        )
