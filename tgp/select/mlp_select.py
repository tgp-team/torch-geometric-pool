        ...
        **kwargs,
        ...
        Args:
            x (~torch.Tensor): Node feature tensor.
                If ``batched_representation=True``, expected shape is
                :math:`\mathbf{X} \in \mathbb{R}^{B \times N \times F}`, with
                batch-size :math:`B`, (maximum) number of nodes :math:`N` for
                each graph, and feature dimension :math:`F`.
                If ``batched_representation=False``, expected shape is
            ...
            batch (~torch.Tensor, optional): The batch vector
                ...
                ``batched_representation=False``. (default: :obj:`None`)

        Returns:
            :class:`~tgp.select.SelectOutput`: The output of :math:`\texttt{select}` operator.
                If ``batched_representation=True``, the assignment matrix :math:`\mathbf{S}`
                has shape :math:`\mathbb{R}^{B \times N \times K}`.
                If ``batched_representation=False``, the assignment matrix :math:`\mathbf{S}`
