...
def make_simple_undirected_graph():
    ...
    col = torch.tensor([1, 2, 3, 0, 0, 1, 2, 3], dtype=torch.long)
    edge_index = torch.stack([row, col], dim=0)
    # Ensure self-loops for maximal_matching code
...
def make_chain_edge_index(N=3):
    # Build a simple 3-node chain 0–1–2 with undirected edges, then add self-loops.
