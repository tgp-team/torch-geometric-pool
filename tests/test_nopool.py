...
def test_nopool_basic_functionality(simple_graph):
    ...
    # Test 4: Test lifting
    x_lifted = pooler(x=out.x, adj=out.edge_index, edge_weight=out.edge_weight, 
                     so=out.so, batch=out.batch, lifting=True)
...
def test_nopool_identity_behavior(simple_graph):
    ...
    pooler = get_pooler("nopool")
    x_pre, adj_pre, mask = pooler.preprocessing(
        edge_index=edge_index, edge_weight=edge_weight, x=x, batch=batch, use_cache=False
    )
...
def test_nopool_in_pooler_list():
    ...
    assert "NoPool" in pooler_map.values().__class__.__name__ or "NoPool" in str(pooler_map.values())
...
def test_nopool_print_signature():
    ...
    assert "IdentityReduce" in pooler_str
    assert "IdentityConnect" in pooler_str
    ...
    assert "IdentityReduce" in reducer_str
    assert "IdentityConnect" in connector_str
...
def test_identity_select_with_x_and_edge_index_uses_x():
    ...
    x = torch.randn(3, 2)
    edge_index = torch.tensor([[0, 1, 2, 3], [1, 2, 3, 0]], dtype=torch.long)  # Would give 4 nodes
