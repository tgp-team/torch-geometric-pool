"""Tests for DMoNPooling pooler."""

import pytest
import torch

from tgp.poolers import DMoNPooling, get_pooler


def test_dmon_initialization():
    """Test DMoNPooling initialization with different parameters."""
    pooler = DMoNPooling(in_channels=16, k=5)
    assert pooler.spectral_loss_coeff == 1.0
    assert pooler.cluster_loss_coeff == 1.0
    assert pooler.ortho_loss_coeff == 0.0
    assert pooler.batched is True

    pooler = DMoNPooling(
        in_channels=16,
        k=5,
        spectral_loss_coeff=0.5,
        cluster_loss_coeff=1.5,
        ortho_loss_coeff=0.5,
        batched=False,
    )
    assert pooler.spectral_loss_coeff == 0.5
    assert pooler.cluster_loss_coeff == 1.5
    assert pooler.ortho_loss_coeff == 0.5
    assert pooler.batched is False


def test_dmon_batched_forward(pooler_test_graph_dense_batch):
    """Test DMoNPooling with batched dense inputs."""
    x, adj = pooler_test_graph_dense_batch
    batch_size, n_nodes, n_features = x.shape
    k = 3

    pooler = DMoNPooling(in_channels=n_features, k=k, batched=True)
    out = pooler(x=x, adj=adj)

    assert out.x.shape == (batch_size, k, n_features)
    assert out.edge_index.shape == (batch_size, k, k)
    assert out.loss is not None
    assert "spectral_loss" in out.loss
    assert "cluster_loss" in out.loss
    assert "ortho_loss" in out.loss


def test_dmon_batched_sparse_output(pooler_test_graph_dense_batch):
    """Test DMoNPooling batched path with sparse output enabled."""
    x, adj = pooler_test_graph_dense_batch
    n_features = x.shape[-1]

    pooler = DMoNPooling(in_channels=n_features, k=3, batched=True, sparse_output=True)
    out = pooler(x=x, adj=adj)
...
def test_dmon_unbatched_single_graph():
    """Test DMoNPooling in unbatched mode with a single graph."""
    ...
    pooler = DMoNPooling(in_channels=n_features, k=k, batched=False)
...
def test_dmon_unbatched_multiple_graphs():
    """Test DMoNPooling in unbatched mode with multiple graphs."""
...
def test_dmon_unbatched_sparse_output():
    """Test DMoNPooling in unbatched mode with sparse output."""
    torch.manual_seed(42)
    n_features = 16
    k = 5
    ...
    pooler = DMoNPooling(in_channels=n_features, k=k, batched=False, sparse_output=True)
    out = pooler(x=x, adj=edge_index, batch=batch)
...
def test_dmon_training_mode(pooler_test_graph_dense_batch, train_mode):
    """Test DMoNPooling gradient computation."""
    x, adj = pooler_test_graph_dense_batch
    n_features = x.shape[-1]

    pooler = DMoNPooling(in_channels=n_features, k=3)
    if train_mode:
        pooler.train()
    else:
        pooler.eval()
...
def test_dmon_get_pooler_u_suffix():
    """Test get_pooler with dmon_u suffix."""
    pooler = get_pooler("dmon_u", in_channels=16, k=5)
    assert pooler.batched is False
...
def test_dmon_lifting_operation(pooler_test_graph_dense_batch):
    """Test the lifting operation in DMoNPooling."""
    x, adj = pooler_test_graph_dense_batch
    n_features = x.shape[-1]
...
def test_dmon_batched_vs_unbatched_loss_equality(pooler_test_graph_dense_batch):
