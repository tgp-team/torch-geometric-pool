...
def test_nopool_basic_functionality(pooler_test_graph_sparse):
    """Test basic NoPool functionality."""
    x, edge_index, edge_weight, batch = pooler_test_graph_sparse
    # Some tests expect [E, 1] edge_weight
...
def test_nopool_identity_behavior(pooler_test_graph_sparse):
...
def test_nopool_precoarsening(pooler_test_graph_sparse_batch):
    ...
    try:
        ...
        assert pooled_batch.so.num_supernodes == num_nodes
        assert pooled_batch.so.num_nodes == num_nodes
