"""Tests for AsymCheegerCutPooling pooler."""

import pytest
import torch

from tgp.poolers import AsymCheegerCutPooling, get_pooler


def test_acc_initialization():
    """Test AsymCheegerCutPooling initialization with different parameters."""
    pooler = AsymCheegerCutPooling(in_channels=16, k=5)
    assert pooler.totvar_coeff == 1.0
    assert pooler.balance_coeff == 1.0
    ...
    assert pooler.totvar_coeff == 0.5
    assert pooler.balance_coeff == 2.0
    assert pooler.batched is False


def test_acc_batched_forward(pooler_test_graph_dense_batch):
    """Test AsymCheegerCutPooling with batched dense inputs."""
    x, adj = pooler_test_graph_dense_batch
    batch_size, n_nodes, n_features = x.shape
    k = 3

    pooler = AsymCheegerCutPooling(in_channels=n_features, k=k, batched=True)
...
def test_acc_unbatched_single_graph():
    ...
    pooler = AsymCheegerCutPooling(in_channels=n_features, k=k, batched=False)
...
def test_acc_unbatched_multiple_graphs():
    """Test AsymCheegerCutPooling in unbatched mode with multiple graphs."""
...
def test_acc_unbatched_sparse_output():
    """Test AsymCheegerCutPooling in unbatched mode with sparse output."""
    torch.manual_seed(42)
    n_features = 16
    k = 5
...
def test_acc_training_mode(pooler_test_graph_dense_batch, train_mode):
    """Test AsymCheegerCutPooling gradient computation."""
    x, adj = pooler_test_graph_dense_batch
    n_features = x.shape[-1]

    pooler = AsymCheegerCutPooling(in_channels=n_features, k=3)
    if train_mode:
        pooler.train()
    else:
        pooler.eval()
...
def test_acc_get_pooler_u_suffix():
    """Test get_pooler with acc_u suffix."""
    pooler = get_pooler("acc_u", in_channels=16, k=5)
    assert pooler.batched is False
...
def test_acc_lifting_operation(pooler_test_graph_dense_batch):
    """Test the lifting operation in AsymCheegerCutPooling."""
    x, adj = pooler_test_graph_dense_batch
    n_features = x.shape[-1]
...
def test_acc_batched_vs_unbatched_loss_equality(pooler_test_graph_dense_batch):
